...
if(!isset($erreur)) //S'il n'y a pas d'erreur, on upload
{
	...
	$numLigne=0;
	  ...
	  {
		$liste = fgetcsv($fp, 4096); // changement de ligne
		...
		if(isset($liste[0])){
					$DEMANDE = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[0]))));
									if ( ($DEMANDE != 'Demande' ) || ($DEMANDE != 'Jira') ) {
										$DEMANDE = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[0]))));
										...
										$ETAT = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[4]))));
										$MOTIF = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[5]))));
										$INTERVENANT = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[6]))));
										$MANDEUR = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[7]))));
										$CREATION1 = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[8]))));
										$MAJ = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[9]))));
										$RESOLUE = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[10]))));
										$COMPOSANT1 = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[11]))));
										$DATE_SOUHAITEE = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[12]))));
										...
										$DATE_SOUHAITEE_2 = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[32]))));
										$DUE_TIME = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[33]))));
										$ENT_METIER = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[34]))));
										$ENV_PHY = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[35]))));
										$ENVIRONNEMENT = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[36]))));
										$ID_APP = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[37]))));
										$FACTU = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[38]))));
										$ANNUL = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[39]))));
										$RESOL = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[40]))));
										$NIV_EXP = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[41]))));
										$NIV_SERV = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[42]))));
										$PERCENT_DONE = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[43]))));
										$REFACTUR = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[44]))));
										$REF_INC_CLI = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[45]))));
										$SITE = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[46]))));
										$TEMPS_JOURS = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[47]))));
										$TEMPS_HEURE = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[48]))));
										$TEMPS_MINUTES = str_replace("&quot;", "",addslashes(trim(htmlentities($liste[49]))));
			}
		}
		...
		if($NUMERO != 'No de demande' ){
			...
			########## Bloc D'ajout dans la base ###########
		        $sql="
										...
										`ID`, `Demande`, `Cle`, `Resume`, `Type`, `Etat`, `Motif`, `Intervenant`, `Demandeur`, `Creation`, `Mise a jour`, `Resolue`, `Composants`, `Date souhaitee 1`, `observateurs`, `Images`, `Estimation originale`, `Estimation restante`, `Temps consacre`, `Ratio du travail reel compare a estimation`, `Sous-taches`, `Demandes liees`, `Descriptif`, `Niveau de securite`, `Progression`, `S Progres`, `S Temps consacre`, `S Estimation restante`, `S Estimation originale`, `libelle`, `Code Convention`, `Code application`, `Composant`, `Date souhaitee 2`, `DueTime`, `Entite metier`, `Env. physique`, `Environnement`, `ID application`, `facturation`, `annulation`, `Resolution`, `Niveau d expertise`, `Niveau de service`, `PercentDone`, `Refacturation`, `RefIncidentclient`, `Site`, `TempsJours`, `TempsHeure`, `TempsMinutes`,  `ENABLE` ) 
										VALUES
										( NULL, '".$DEMANDE."', '".$CLE."', '".$RESUME."', '".$TYPE_DEMANDE."', '".$ETAT."', '".$MOTIF."', '".$INTERVENANT."', '".$MANDEUR."', '".$CREATION1."', '".$MAJ."', '".$RESOLUE."', '".$COMPOSANT1."', '".$DATE_SOUHAITEE."', '".$OBSERVATEUR."', '".$IMAGE."', '".$ESTIM_O."', '".$ESTIM_R."', '".$TEMP_C."', '".$RATIO."', '".$SS_TACHE."', '".$D_LIEE."', '".$DESC."', '".$N_SECU."', '".$PROGRESSION."', '".$SOM_PROGRES."', '".$SOM_TEMP_C."', '".$SOM_EST_R."', '".$SOM_EST_O."', '".$LIBELLE."', '".$CODE_CONV."', '".$CODE_APP."', '".$COMPOSANT2."', '".$DATE_SOUHAITEE_2."', '".$DUE_TIME."', '".$ENT_METIER."', '".$ENV_PHY."', '".$ENVIRONNEMENT."', '".$ID_APP."', '".$FACTU."', '".$ANNUL."', '".$RESOL."', '".$NIV_EXP."', '".$NIV_SERV."', '".$PERCENT_DONE."', '".$REFACTUR."', '".$REF_INC_CLI."', '".$SITE."', '".$TEMPS_JOURS."', '".$TEMPS_HEURE."', '".$TEMPS_MINUTES."', 1 ); ";
		     mysql_query($sql, $mysql_link) or die('Erreur SQL !'.$sql.''.mysql_error());
	        }
	     }
	  }
	  ...
	  $OptTable=mysql_query("OPTIMIZE TABLE `indicateur_darwin_extract_brut`",$mysql_link);
        ...
        url=("./index.php?ITEM=indicateur_darwin_traitement");
        window.location=url;
        </script>
        ';
...
echo '
<form method="POST" action="./index.php?ITEM=indicateur_darwin_upload" enctype="multipart/form-data">
