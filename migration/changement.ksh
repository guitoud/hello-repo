...
then
	...
	mysql --user=${MYSQL_USER} --password=${MYSQL_PASS} --database=${MYSQL_BASE}<${DOSSIER_MIGRATION}/changement.sql
fi


cd ${DOSSIER}
mkdir ${DOSSIER}/changement/
mkdir ${DOSSIER}/old_changement/
cp ${DOSSIER_MIGRATION}changement/* ${DOSSIER}/changement/
