...
//		}
	...
	case "oui-non": 
		...
		$CHANGEMENT_FAR_CONFIG_ID_COMMENTAIRE=$CHANGEMENT_FAR_CONFIG_ID.'_COMMENTAIRE';
		if(!isset($tab_var[$LISTE_CONFIG_COMMENTAIRE])){$tab_var[$LISTE_CONFIG_COMMENTAIRE]='';}
		$CHANGEMENT_FAR_CONFIG[$LISTE_CONFIG_COMMENTAIRE]=addslashes(trim(htmlentities($tab_var[$LISTE_CONFIG_COMMENTAIRE])));
		$CHANGEMENT_FAR_CONFIG_STOP[$CHANGEMENT_FAR_CONFIG_ID]=0;
//		if($CHANGEMENT_FAR_CONFIG_OBLIGATOIRE=='oui'){
//			if($CHANGEMENT_FAR_CONFIG[$CHANGEMENT_FAR_CONFIG_ID]==''){
//				$CHANGEMENT_FAR_CONFIG_STOP[$CHANGEMENT_FAR_CONFIG_ID]=1;
...
//		}
	...
	case "risque": 
		...
		if(!isset($tab_var[$LISTE_CONFIG_COMMENTAIRE])){$tab_var[$LISTE_CONFIG_COMMENTAIRE]='';}
		$CHANGEMENT_FAR_CONFIG[$LISTE_CONFIG_COMMENTAIRE]=addslashes(trim(htmlentities($tab_var[$LISTE_CONFIG_COMMENTAIRE])));
		$sql="INSERT INTO `changement_far` (`CHANGEMENT_FAR_ID` ,`CHANGEMENT_LISTE_ID` ,`CHANGEMENT_FAR_CONFIG_ID` ,`CHANGEMENT_FAR_INFO_AUTRE_ID` ,`CHANGEMENT_FAR_VALEUR`,`CHANGEMENT_FAR_COMMENTAIRE`,`ENABLE` ) VALUES (NULL , '".$CHANGEMENT_ID."', '".$CHANGEMENT_FAR_CONFIG_ID."', '".$CHANGEMENT_FAR_INFO_AUTRE_ID."', '".$CHANGEMENT_FAR_CONFIG[$CHANGEMENT_FAR_CONFIG_ID]."','".$CHANGEMENT_FAR_CONFIG[$LISTE_CONFIG_COMMENTAIRE]."', '0');";
		mysql_query($sql) or die('Erreur SQL !'.$sql.''.mysql_error());
		$TABLE_SQL_SQL='changement_far';       
		historique_sql_new($sql,$TABLE_SQL_SQL,'INSERT');
	break;
...
if($total_ligne_rq_info_config!=0){
    ...
    AND `CHANGEMENT_FAR_INFO_AUTRE_ID` ='".$CHANGEMENT_FAR_INFO_AUTRE_ID."' 
    AND `CHANGEMENT_FAR_VALEUR` ='".$CHANGEMENT_FAR_CONFIG[$CHANGEMENT_FAR_CONFIG_ID]."'  
    AND `CHANGEMENT_FAR_COMMENTAIRE` ='".$CHANGEMENT_FAR_CONFIG[$LISTE_CONFIG_COMMENTAIRE]."'
    AND `ENABLE` ='0'
    LIMIT 1";
    ...
    if($total_ligne_rq_info==0){
      ...
      UPDATE `changement_far` SET 
      `CHANGEMENT_FAR_VALEUR`= '".$CHANGEMENT_FAR_CONFIG[$CHANGEMENT_FAR_CONFIG_ID]."',
      `CHANGEMENT_FAR_COMMENTAIRE`= '".$CHANGEMENT_FAR_CONFIG[$LISTE_CONFIG_COMMENTAIRE]."'
      WHERE `CHANGEMENT_LISTE_ID` ='".$CHANGEMENT_ID."'
      AND `CHANGEMENT_FAR_CONFIG_ID`='".$CHANGEMENT_FAR_CONFIG_ID."'
      AND `CHANGEMENT_FAR_INFO_AUTRE_ID`= '".$CHANGEMENT_FAR_INFO_AUTRE_ID."'
      AND `ENABLE`='0'
      ...
      $TRACE_ETAT=$CHANGEMENT_STATUS_ID;
			...
			if($CHANGEMENT_STATUS == 'Brouillon' ){
				...
				if($total_ligne_rq_info_etat==0){
					$sql="
					UPDATE `changement_far` 
					SET `CHANGEMENT_FAR_ETAT` = 'B' 
...
##########################################
  ...
  if($tab_var['btn']=="Inscription"){
	...
	if($STOP==0){
		...
		WHERE `CHANGEMENT_RESSOURCES_ETAT` = 'I'
		...
		if($total_ligne_rq_info_etat==0){
			...
			if($total_ligne_rq_info_etat==0){ 
				echo '
				<script language="JavaScript">
				url=("./index.php?ITEM=changement_Ajout_Ressources&action=Ajout&ID='.$ID.'");
				window.location=url;
				</script>
				';
			}else{
...
if($action=="Info"){
	...
	if($total_ligne_rq_info_far!=0){
		do {
			...
			$LISTE_CONFIG_COMMENTAIRE=$CHANGEMENT_FAR_CONFIG_ID.'_COMMENTAIRE';
			$CHANGEMENT_FAR_CONFIG[$LISTE_CONFIG_COMMENTAIRE]=$tab_rq_info_far['CHANGEMENT_FAR_COMMENTAIRE'];      
...
<form method="post" name="frm_inter" id="frm_inter" action="./index.php?ITEM='.$_GET['ITEM'].'">
  ...
  $rq_info_id="
    ...
    if($action=="Ajout"){
      echo '
      Cr&eacute;ation de la Fiche d\'Analyse de Risque du changement n&deg; <a href="#" class="infobulledroite">'.$ID.'<span>'.$AFF_SPAN.'</span></a>';
    }
    if($action=="Modif"){
      if($CHANGEMENT_STATUS=='Brouillon'){
        echo 'Modification de la Fiche d\'Analyse de Risque du changement n&deg; <a href="#" class="infobulledroite">'.$ID.'<span>'.$AFF_SPAN.'</span></a>';
      }else{
        if($ROLE==0){
          echo 'Modification de la Fiche d\'Analyse de Risque du changement n&deg; <a href="#" class="infobulledroite">'.$ID.'<span>'.$AFF_SPAN.'</span></a>';
        }else{
          echo 'Information de la Fiche d\'Analyse de Risque du changement n&deg; <a href="#" class="infobulledroite">'.$ID.'<span>'.$AFF_SPAN.'</span></a>';
        }
      }
    }
    if($action=="Info"){
    ...
    $rq_info_far_lib="
      ...
      <tr align="center" class="titre">
		...
		if($action=="Ajout"){
              	...
              	`changement_far_config`.`CHANGEMENT_FAR_CONFIG_INFO` , 
              	`changement_far_config`.`CHANGEMENT_FAR_CONFIG_COMMENTAIRE` ,
	      ...
	      if($total_ligne_rq_info_far!=0){
	        do {
	        	...
	        	$CHANGEMENT_FAR_CONFIG_COMMENTAIRE=$tab_rq_info_far['CHANGEMENT_FAR_CONFIG_COMMENTAIRE'];
	        	$CHANGEMENT_FAR_CONFIG_INFO=$tab_rq_info_far['CHANGEMENT_FAR_CONFIG_INFO'];
			...
			if(!isset($CHANGEMENT_FAR_CONFIG[$CHANGEMENT_FAR_CONFIG_ID])){$CHANGEMENT_FAR_CONFIG[$CHANGEMENT_FAR_CONFIG_ID]='';}
	        	...
	        	if(!isset($CHANGEMENT_FAR_CONFIG[$CHANGEMENT_FAR_CONFIG_ID])){$CHANGEMENT_FAR_CONFIG[$CHANGEMENT_FAR_CONFIG_ID]='';}
	        	$LISTE_CONFIG_COMMENTAIRE=$CHANGEMENT_FAR_CONFIG_ID.'_COMMENTAIRE';
	        	if(!isset($CHANGEMENT_FAR_CONFIG[$LISTE_CONFIG_COMMENTAIRE])){$CHANGEMENT_FAR_CONFIG[$LISTE_CONFIG_COMMENTAIRE]='';}
	       $j++;
		  ...
		  echo '
            ...
            case "oui-non": 
              $AFF_SPAN_AIDE='';
              if($CHANGEMENT_FAR_CONFIG_INFO!=''){$AFF_SPAN_AIDE.=$CHANGEMENT_FAR_CONFIG_INFO.'</BR>';}
              echo '
              ...
              echo '>';
              $LISTE_CONFIG_COMMENTAIRE=$CHANGEMENT_FAR_CONFIG_ID.'_COMMENTAIRE';
              if($CHANGEMENT_FAR_CONFIG_COMMENTAIRE=='oui'){
                  	if($action!='Info'){
                  		echo '&nbsp;-&nbsp;<input '.$readonly_var.' id="CHANGEMENT_FAR_CONFIG_ID_'.$CHANGEMENT_FAR_CONFIG_ID.'_COMMENTAIRE" name="CHANGEMENT_FAR_CONFIG_ID_'.$CHANGEMENT_FAR_CONFIG_ID.'_COMMENTAIRE" type="text" value="'.stripslashes($CHANGEMENT_FAR_CONFIG[$LISTE_CONFIG_COMMENTAIRE]).'" size="80" />';
                	}else{
                		echo '&nbsp;:&nbsp;'.stripslashes($CHANGEMENT_FAR_CONFIG[$LISTE_CONFIG_COMMENTAIRE]);
                	}
                  }
              if($AFF_SPAN_AIDE!=''){
            ...
            $AFF_SPAN_AIDE.='1 Faible < > 4 Fort';
              ...
              if($CHANGEMENT_FAR_CONFIG_COMMENTAIRE=='oui'){
                  	...
                  	if($action!='Info'){
                  		echo '&nbsp;-&nbsp;<input '.$readonly_var.' id="CHANGEMENT_FAR_CONFIG_ID_'.$CHANGEMENT_FAR_CONFIG_ID.'_COMMENTAIRE" name="CHANGEMENT_FAR_CONFIG_ID_'.$CHANGEMENT_FAR_CONFIG_ID.'_COMMENTAIRE" type="text" value="'.stripslashes($CHANGEMENT_FAR_CONFIG[$LISTE_CONFIG_COMMENTAIRE]).'" size="50" />';
                	}else{
                		echo '&nbsp;:&nbsp;'.stripslashes($CHANGEMENT_FAR_CONFIG[$LISTE_CONFIG_COMMENTAIRE]);
                	}
                  }
            ...
            if($action!='Info'){
              echo '<textarea '.$readonly_var.' id="CHANGEMENT_FAR_CONFIG_ID_'.$CHANGEMENT_FAR_CONFIG_ID.'" name="CHANGEMENT_FAR_CONFIG_ID_'.$CHANGEMENT_FAR_CONFIG_ID.'" cols="70" rows="5">'.stripslashes($CHANGEMENT_FAR_CONFIG[$CHANGEMENT_FAR_CONFIG_ID]).'</textarea>';
        	}else{
