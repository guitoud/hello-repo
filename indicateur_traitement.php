...
$GROUPE_DE_TRAVAIL='';
$IMPLANTATION_GEO='';
$IMPLANTATION_DEMANDE='';
$MANAGER_GROUP_TRAVAIL='';
$MESSAGE='';
$NOM_APPLICATION='';
$ORDRE_INTERVENTION='';
$REF_DEROGATION='';
...
// gestion des corrections en base
function Return_cas_particulier($REF,$NOM_VAR,$VAR,$mysql_link)
{	        
	$rq_info="
	SELECT `INDICATEUR_CAS_PARTICULIER_REF`, `INDICATEUR_CAS_PARTICULIER_TYPE`, `INDICATEUR_CAS_PARTICULIER_VALEUR`
	FROM `indicateur_cas_particulier`
	WHERE  `INDICATEUR_CAS_PARTICULIER_TYPE`='".$NOM_VAR."'
	AND `INDICATEUR_CAS_PARTICULIER_REF`='".$REF."'
	AND `ENABLE`='0'
	LIMIT 1";
	$res_rq_info = mysql_query($rq_info, $mysql_link) or die(mysql_error());
	$tab_rq_info = mysql_fetch_assoc($res_rq_info);
	$total_ligne_rq_info=mysql_num_rows($res_rq_info);
	if($total_ligne_rq_info==0){
		$INDICATEUR_CAS_PARTICULIER_VALEUR=$VAR;
	}else{
		$INDICATEUR_CAS_PARTICULIER_VALEUR=$tab_rq_info['INDICATEUR_CAS_PARTICULIER_VALEUR'];
	}	
        mysql_free_result($res_rq_info);
        return $INDICATEUR_CAS_PARTICULIER_VALEUR;
}
...
// on traite l import
	  ...
	  $rq_info_extract="SELECT * FROM `indicateur_extract_brut`"; 
		...
		$rq_info_cas_particulier="
		SELECT `INDICATEUR_CAS_PARTICULIER_ID` 
		FROM `indicateur_cas_particulier` 
		WHERE
		`INDICATEUR_CAS_PARTICULIER_REF`='".$REF."'
		AND `ENABLE`='0'";
		$res_rq_info_cas_particulier = mysql_query($rq_info_cas_particulier, $mysql_link) or die(mysql_error());
		$tab_rq_info_cas_particulier = mysql_fetch_assoc($res_rq_info_cas_particulier);
		$total_ligne_rq_info_cas_particulier=mysql_num_rows($res_rq_info_cas_particulier);
		mysql_free_result($res_rq_info_cas_particulier);
		if($total_ligne_rq_info_cas_particulier!=0){
			$DATE_FIN_RELLE=Return_cas_particulier($REF,"DATE_FIN_RELLE",$DATE_FIN_RELLE,$mysql_link);
			$RESUME=Return_cas_particulier($REF,"RESUME",$RESUME,$mysql_link);
			$ENVIRONNEMENT=Return_cas_particulier($REF,"ENVIRONNEMENT",$ENVIRONNEMENT,$mysql_link);
			$RESUME=Return_cas_particulier($REF,"RESUME",$RESUME,$mysql_link);
		}
		...
		// on traite l environnement
			   ...
			   // on traite la nature
		# si on est avant 12/2011 on prend
		if($DATE_INDICATEUR < 201112 ){
			# Pr&eacute;inscription LUP/Version - BORDEAUX passe de I a V
			if($NATURE == 'Pr&eacute;inscription LUP/Version - BORDEAUX'){
				$NATURE='I';
			}else{
		...
		// cas particulier avant nomenclature
		if($total_ligne_rq_info_cas_particulier!=0){
			$ACTION_NEW=Return_cas_particulier($REF,"ACTION_NEW",$ACTION_NEW,$mysql_link);
			$ENV=Return_cas_particulier($REF,"ENV",$ENV,$mysql_link);
		}		 
		// fin cas particulier
		...
		// on traire la valeur
			list($VALEUR,$INDICATEUR_REGLE_ID)=Return_poids_valeur($MOT_CLES,$mysql_link);
			  ...
			  // echo '</BR>';
			   ...
			   $sql="
			   INSERT INTO `indicateur_calcul` (`ID` ,`REF` , `DATE` ,`ACTION` ,`STATUS` ,`ACTEUR` ,`ENV` ,`NATURE` ,`DEMANDE` ,`DUREE` ,`DATE_ANNEE` ,`DATE_MOIS` ,`DATE_SEMAINE` ,`DATE_JOUR` ,`DATE_JOURSEM` ,`DATE_PREVUE` ,`EN_VERSION` ,`EN_HVP` ,`EN_HVH` ,`APPLI` ,`SOGETI`,`POIDS`,`NIVEAU` ,`VALEUR` ,`INDICATEUR_REGLE_ID` ,`DATE_INDICATEUR`,`ACTION_NEW`,`INDICATEUR_ACTION_ID` )
			   			   VALUES 
			   ( NULL, '".$REF."', '".$DATE."', '".$ACTION."', '".$STATUS."', '".$ACTEUR."', '".$ENV."', '".$NATURE."', '".$DEMANDE."', '".$DUREE."', '".$DATE_ANNEE."', '".$DATE_MOIS."', '".$DATE_SEMAINE."', '".$DATE_JOUR."', '".$DATE_JOURSEM."', '".$DATE_PREVUE."', '".$EN_VERSION."', '".$EN_HVP."', '".$EN_HVH."', '".$APPLI."', '".$SOGETI."', '".$POIDS."', '".$NIVEAU."','".$VALEUR."','".$INDICATEUR_REGLE_ID."', '".$DATE_INDICATEUR."', '".$ACTION_NEW."', '".$ACTION_ID."');
			   ";
			   mysql_query($sql, $mysql_link) or die('Erreur SQL !'.$sql.''.mysql_error());
