...
then
	...
	mysql --user=${MYSQL_USER} --password=${MYSQL_PASS} --database=${MYSQL_BASE}<${DOSSIER_MIGRATION}/ctlm.sql
fi


cd ${DOSSIER}
mkdir ${DOSSIER}/corres_CTLM/
cp ${DOSSIER_MIGRATION}corres_CTLM/* ${DOSSIER}/corres_CTLM/
cp ${DOSSIER_MIGRATION}scripts/* ${DOSSIER}/scripts/
