...
$Var_NOM_SITE='COMMUNICATION DPI';
$Var_LIB_NOM_SITE='Bienvenue sur le Portail de DPI-DDR';
...
function historique_sql_new($sql, $TABLE_SQL_SQL, $TYPE_ORDRE)
{
    ...
    $sql_sql="INSERT INTO `moteur_historique` 
        (`HISTORIQUE_ID` ,`HISTORIQUE_DATE` ,`HISTORIQUE_LOGIN` ,`HISTORIQUE_TABLE`
        ,`HISTORIQUE_TYPE`,`HISTORIQUE_SQL`)
        VALUES (
  NULL , '".$date_sql_sql."', '".$LOGIN_sql_sql."', 
  '".$TABLE_SQL_SQL."', '".$TYPE_SQL."', '".$sql_mod."'
  );
  ";
...
function moteur_trace($TRACE_CATEGORIE, $TRACE_TABLE, $TRACE_REF_ID, $TRACE_ACTION, $TRACE_ETAT)
{
    ...
    NULL, '".$UTILISATEUR_ID."',
    '".$DATE_TRACE."','".$DATE_TRACE_TRI."','".$TRACE_CATEGORIE."','"
        .$TRACE_TABLE."', '".$TRACE_REF_ID."', '"
        .$TRACE_ACTION."', '".$TRACE_ETAT."');";
...
function test_date_Ymd($date_rapport)
{// retourne 0 si jour normal 1 si jour feriee
    ...
    if ($jour == $jour_paque && $mois == $mois_paque) {
        $test_date=1;
    } // jour de paques
    ...
    if ($jour == $jour_lundi_paque && $mois == $mois_lundi_paque) {
        $test_date=1;
    } // Lundi de Paques
    ...
    if ($jour == $jour_Ascension && $mois == $mois_Ascension) {
        $test_date=1;
    } // Ascension
    ...
    if ($jour == $jour_Pentecote && $mois == $mois_Pentecote) {
        $test_date=1;
    } // Pentecôte
    ...
    if ($test_date==1) {
        ...
        {
            case "Mon":
                return date("Ymd", mktime(0, 0, 0, $mois, $jour-1, $annee));
            break;
            case "Tue":
                return date("Ymd", mktime(0, 0, 0, $mois, $jour-1, $annee));
            break;
            ...
            break;
            case "Sat":
                return date("Ymd", mktime(0, 0, 0, $mois, $jour-2, $annee));
            break;
            case "Sun":
                return date("Ymd", mktime(0, 0, 0, $mois, $jour-3, $annee));
            break;
        }
    } else {
...
function test_date_dmY($date_rapport)
{// retourne 0 si jour normal 1 si jour feriee
    ...
    if ($test_date==1) {
        ...
        {
            case "Mon":
                return date("dmY", mktime(0, 0, 0, $mois, $jour-1, $annee));
            break;
            case "Tue":
                return date("dmY", mktime(0, 0, 0, $mois, $jour-1, $annee));
            break;
            ...
            break;
            case "Sat":
                return date("dmY", mktime(0, 0, 0, $mois, $jour-2, $annee));
            break;
            case "Sun":
                return date("dmY", mktime(0, 0, 0, $mois, $jour-3, $annee));
            break;
        }
    } else {
        $NomJour=date('D', mktime(12, 0, 0, $mois, $jour, $annee));
        ...
        {
            case "Mon":
                return date("dmY", mktime(0, 0, 0, $mois, $jour, $annee));
            break;
            case "Tue":
                return date("dmY", mktime(0, 0, 0, $mois, $jour, $annee));
            break;
...
{// change le format datetime sql en une date de type : JJ/MM/AAAA XX:XX:XX
