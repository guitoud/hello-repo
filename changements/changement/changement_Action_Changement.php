...
function enregistrement_modif($ID,$tab_var,$ENV,$STOP,$type_btn,$mysql_link){ 
    $action='Modif';
    $DATE_DU_JOUR=date("Ymd");
    $CHANGEMENT_ID=$ID;
...
if(empty($tab_var['btn'])){
  ...
  if($tab_var['btn']=="Enregistrer"){
    ...
    # Cas Modif
    if($tab_var['action']=="Modif"){
    	$type_btn='';
    	enregistrement_modif($ID,$tab_var,$ENV,$STOP,$type_btn,$mysql_link);
    }
  # Cas Ajout
  if($tab_var['action']=="Ajout"){
...
// ajout date pour le calendrier    	
	...
	historique_sql_new($sql,$TABLE_SQL_SQL,'OPTIMIZE');
        ...
        echo '
        <script language="JavaScript">
        url=("./index.php?ITEM=changement_Ajout_FAR&action=Ajout&ID='.$CHANGEMENT_ID.'");
        window.location=url;
        </script>
        ';
	}else{
...
# Cas Fiche Bilan
  ...
  # Cas Fiche d Analyse de Risque
  if($tab_var['btn']=="Fiche d Analyse de Risque"){
  	$type_btn='Fiche d Analyse de Risque';
  	enregistrement_modif($ID,$tab_var,$ENV,$STOP,$type_btn,$mysql_link);
  ...
  # Cas Fiche Ressources
  if($tab_var['btn']=="Fiche Ressources"){
  	$type_btn='Fiche Ressources';
  	enregistrement_modif($ID,$tab_var,$ENV,$STOP,$type_btn,$mysql_link);
  	if(isset($tab_var['RESSOUCES_action'])){
	  $RESSOUCES_action=$tab_var['RESSOUCES_action'];
	}else{
	  $RESSOUCES_action='Info';
	}
	switch ($RESSOUCES_action)
	{
	  case "Ajout": 
		$RESSOUCES_action='Ajout';
            	$RESSOUCES_ITEM='changement_Ajout_RESSOURCES';
	  break;
	  case "Modif": 
		$RESSOUCES_action='Modif';
            	$RESSOUCES_ITEM='changement_Modif_RESSOURCES';
	  break;
	  case "Info": 
		$RESSOUCES_action='Info';
		$RESSOUCES_ITEM='changement_Info_RESSOURCES';
	  break;
	  default:
		$RESSOUCES_action='Info';
		$RESSOUCES_ITEM='changement_Info_RESSOURCES';
  	...
  	echo '
        <script language="JavaScript">
        url=("./index.php?ITEM='.$RESSOUCES_ITEM.'&action='.$RESSOUCES_action.'&ID='.$ID.'");
        window.location=url;
        </script>
        ';
  }
...
if($_GET['action']=="Modif" || $_GET['action']=="Info"){
  ...
  mysql_free_result($res_rq_info_id);
    ...
    switch ($CHANGEMENT_STATUS){
      ...
      AND `ENABLE` = '0'
      ";
      $res_rq_ressources_info = mysql_query($rq_ressources_info, $mysql_link) or die(mysql_error());
      $tab_rq_ressources_info = mysql_fetch_assoc($res_rq_ressources_info);
      $total_ligne_rq_ressources_info=mysql_num_rows($res_rq_ressources_info);
      mysql_free_result($res_rq_ressources_info);
      $RESSOUCES_action='Ajout';
      $RESSOUCES_lib='Cr&eacute;ation de la Fiche de ressouces';
      $RESSOUCES_ITEM='changement_Ajout_Ressources';
      if($action=="Info"){
	      $RESSOUCES_action='';
	      $RESSOUCES_lib='';
      ...
      if($total_ligne_rq_ressources_info > 1){
        $RESSOUCES_action='Modif';
        $RESSOUCES_lib='Modification de la Fiche de ressouces';
        $RESSOUCES_ITEM='changement_Modif_Ressources';
        if($action=="Info"){
		$RESSOUCES_action='Info';
		$RESSOUCES_lib='Voir la Fiche de ressouces';
		$RESSOUCES_ITEM='changement_Info_Ressources';
        }
      }
	...
	$RESSOUCES_ITEM='changement_Info_Ressources';
      ...
      if($total_ligne_rq_far_info > 1){
		...
		if($total_ligne_rq_info!=0){
			...
			$Date_Inter_Debut_bdd=$tab_rq_info['CHANGEMENT_LISTE_DATE_DEBUT'];
			...
			$Date_Inter_Fin_bdd=$tab_rq_info['CHANGEMENT_LISTE_DATE_FIN'];
...
<form method="post" name="frm_inter" id="frm_inter" action="./index.php?ITEM='.$_GET['ITEM'].'">
  ...
  echo '
    <tr class="'.$class.'">
    <td align="center" colspan="4">
    <table class="table_inc" cellspacing="2">
    <tr>
     <td align="left">&nbsp;Date et Heure de D&eacute;but&nbsp;*&nbsp;</td>
     <td align="left">';
  ...
  echo '
   ...
   <tr class="'.$class.'">
    ...
    if($action!='Info'){
     echo '
    <input id="txt_lib" name="txt_lib" '.$readonly_var.' type="text" value="'.stripslashes($lib).'" size="100" maxlength="150"/>';
	}else{
  ...
  ORDER BY `changement_liste_config`.`CHANGEMENT_LISTE_CONFIG_ORDRE`";
	...
	if($total_ligne_rq_info_config!=0){
		...
		if(!isset($CHANGEMENT_LISTE_CONFIG[$CHANGEMENT_LISTE_CONFIG_ID])){$CHANGEMENT_LISTE_CONFIG[$CHANGEMENT_LISTE_CONFIG_ID]='';}
		  ...
		  echo '
            ...
            case "varchar": 
    if($action!='Info'){
              echo '<input '.$readonly_var.' id="CHANGEMENT_LISTE_CONFIG_ID_'.$CHANGEMENT_LISTE_CONFIG_ID.'" name="CHANGEMENT_LISTE_CONFIG_ID_'.$CHANGEMENT_LISTE_CONFIG_ID.'" type="text" value="'.stripslashes($CHANGEMENT_LISTE_CONFIG[$CHANGEMENT_LISTE_CONFIG_ID]).'" size="100" maxlength="150"/>';
        }else{
        echo stripslashes($CHANGEMENT_LISTE_CONFIG[$CHANGEMENT_LISTE_CONFIG_ID]);
            ...
            case "text": 
            if($action!='Info'){
              echo '<textarea '.$readonly_var.' id="CHANGEMENT_LISTE_CONFIG_ID_'.$CHANGEMENT_LISTE_CONFIG_ID.'" name="CHANGEMENT_LISTE_CONFIG_ID_'.$CHANGEMENT_LISTE_CONFIG_ID.'" cols="74" rows="8">'.stripslashes($CHANGEMENT_LISTE_CONFIG[$CHANGEMENT_LISTE_CONFIG_ID]).'</textarea>';
        }else{
   ...
   if($action=='Modif'||$action=="Info"){
	   ...
	   echo '
	   <tr class="'.$class.'">
	    <td align="left">&nbsp;Fiche d\'Analyse de Risque &nbsp;</td>
	    <td align="left" colspan="3"><a class="LinkDef" href="./index.php?ITEM='.$FAR_ITEM.'&action='.$FAR_action.'&ID='.$ID.'">&nbsp;'.$FAR_lib.'&nbsp;</a></td>
	   </tr>';
		}
	   }
	   if($RESSOUCES_ITEM!=''){
	   	if($RESSOUCES_action=="Info"){
	   $j++;
	   if ($j%2) { $class = "pair";}else{$class = "impair";} 
	   echo '
	   <tr class="'.$class.'">
	    <td align="left">&nbsp;Fiche de Ressouces&nbsp;</td>
	    <td align="left" colspan="3"><a class="LinkDef" href="./index.php?ITEM='.$RESSOUCES_ITEM.'&action='.$RESSOUCES_action.'&ID='.$ID.'">&nbsp;'.$RESSOUCES_lib.'&nbsp;</a></td>
	   </tr>';
		}
	   }
  }
		...
		case "Brouillon": 
			...
			if($FAR_ITEM!=''){
			   	if($FAR_action!="Info"){
			   		...
			   		echo '<input name="btn" type="submit" id="btn" value="Fiche d Analyse de Risque">';
			...
			if($RESSOUCES_ITEM!=''){
			   	if($RESSOUCES_action!="Info"){
			   		if($NB_BTN_AUTRE==0){echo '&nbsp;-&nbsp;';}
			   		echo '<input name="btn" type="submit" id="btn" value="Fiche Ressources">';
			   		echo '<input type="hidden" name="RESSOUCES_action" value="'.$RESSOUCES_action.'">'; 
			   		$NB_BTN_AUTRE=$NB_BTN_AUTRE+1;
				}
			}
		break;
