...
function Recup_date_php()
{
  ...
  $mois = date("m"); 
  $annee = date("Y");
# mettre les infos pour faire un mail antidater et faire la modif dans la table archives_mail
#	$nomjour = "Monday";
#	$jour = "31";
#	$mois = "01";
#	$annee = "2011";
...
function Archive_Mail_FULL($UTILISATEUR_ID,$TYPE_MAIL,$DOSSIER,$FICHIER, $EMETTEUR, $REPLY, $DEST_A, $DEST_CC, $DEST_CCI)
{
	$query = "INSERT INTO `archives_mail` (`ID` ,`DATE` ,`UTILISATEUR_ID` ,`TYPE_MAIL`,`DOSSIER`,`FICHIER`,`EMETTEUR`,`REPLY`,`DEST_A`,`DEST_CC`,`DEST_CCI` )";
	$query .= "VALUES (NULL , '".date("YmdHis")."', '".$UTILISATEUR_ID."', '".$TYPE_MAIL."', '".$DOSSIER."', '".$FICHIER."', '".$EMETTEUR."', '".$REPLY."', '".$DEST_A."', '".$DEST_CC."', '".$DEST_CCI."');";
	@mysql_query($query) or die("probleme pendant l'insertion en Base sur la table archives_mail");
	@mysql_query("OPTIMIZE TABLE `archives_mail`") or die("probleme pendant l'insertion en Base sur la table archives_mail");
}
...
function Recherche_SQL_LISTE_MAIL($TYPE_MAIL)
{
	if(substr_count($TYPE_MAIL, '_DEST_')==1){
		$LIMIT_SQL="";
	}else{
		$LIMIT_SQL="LIMIT 1";
	}
      	$rq_info="
	SELECT 
	`configuration_mail`.`CONFIGURATION_MAIL_LIB`, 
	`configuration_mail`.`CONFIGURATION_MAIL_MAIL`
	FROM `configuration_mail`,`configuration_mail_categorie`
	WHERE
	`configuration_mail_categorie`.`CONFIGURATION_MAIL_CATEGORIE_ID`=`configuration_mail`.`CONFIGURATION_MAIL_CATEGORIE_ID` 
	AND `configuration_mail_categorie`.`CONFIGURATION_MAIL_CATEGORIE_LIB`='".$TYPE_MAIL."'
	AND `configuration_mail`.`ENABLE` ='0'
	AND `configuration_mail_categorie`.`ENABLE` ='0'
	ORDER BY `configuration_mail_categorie`.`CONFIGURATION_MAIL_CATEGORIE_LIB`,`configuration_mail`.`CONFIGURATION_MAIL_MAIL`
	".$LIMIT_SQL."
	";
	...
	if ($total_ligne_rq_info==0){
		...
		$MAIL_LIB='';
		do {
			$MAIL_ADR.=stripslashes($tab_rq_info['CONFIGURATION_MAIL_MAIL']).';';
			$MAIL_LIB.=stripslashes($tab_rq_info['CONFIGURATION_MAIL_LIB']).';';
  ...
  return array($MAIL_ADR,$MAIL_LIB);
}
