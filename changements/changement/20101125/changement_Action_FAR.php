...
if($total_ligne_rq_info_config!=0){
	...
	$rq_info_status="
		...
		AND `ENABLE` ='0'
		LIMIT 1";
		$res_rq_info_CHANGEMENT_STATUS = mysql_query($rq_info_CHANGEMENT_STATUS, $mysql_link) or die(mysql_error());
		$tab_rq_info_CHANGEMENT_STATUS = mysql_fetch_assoc($res_rq_info_CHANGEMENT_STATUS);
		$total_ligne_rq_info_CHANGEMENT_STATUS=mysql_num_rows($res_rq_info_CHANGEMENT_STATUS);
		$CHANGEMENT_STATUS=$tab_rq_info_CHANGEMENT_STATUS['CHANGEMENT_STATUS'];
		mysql_free_result($res_rq_info_CHANGEMENT_STATUS);
		if($CHANGEMENT_STATUS=='Brouillon'){
			$nb_modif=$nb_modif+1;
			$rq_info_status="
			SELECT `CHANGEMENT_STATUS_ID` 
...
if($STOP==1){
   ...
   <tr class="titre">
	...
	<input type="hidden" name="CHANGEMENT_FAR_INSCRIPTION" value="Oui">
